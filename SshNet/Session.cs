...
namespace Renci.SshNet
{
    ...
    public partial class Session : IDisposable
    {
        ...
        /// <summary>
        /// The reader for the connection socket.
        /// </summary>
        private DataReader _socketReader;

        /// <summary>
        /// The writer for the connection socket.
        /// </summary>
        ...
        private CryptographicKey _serverMac;

        private CryptographicKey _clientMac;
        ...
        public bool IsConnected
        {
            get
            {
                return (!this._isDisconnecting && this._socket != null && this._isAuthenticated && this._messageListenerCompleted != null)/* && !(this._socket.Poll(10, SelectMode.SelectRead))*/;
            }
        }
        ...
        internal Session(ConnectionInfo connectionInfo)
        {
            ...
            this.ClientVersion = string.Format(CultureInfo.CurrentCulture, "SSH-2.0-RemoteTerminal.SshClient.1.0.0");
        }
        ...
        {
            if (this._socket == null || this._socket.OutputStream == null)
                return;
            ...
            //  Add random padding
            IBuffer buffer = CryptographicBuffer.GenerateRandom(paddingLength);
            byte[] paddingBytes;
            CryptographicBuffer.CopyToByteArray(buffer, out paddingBytes);
            ...
            lock (this._socket)
            {
                ...
                {
                    IBuffer clientHashBuffer = CryptographicEngine.Sign(this._clientMac, CryptographicBuffer.CreateFromByteArray(hashData.ToArray()));
                    byte[] hash;
                    CryptographicBuffer.CopyToByteArray(clientHashBuffer, out hash);

                    var data = new byte[packetData.Length + this._clientMac.KeySize];
        ...
        {
            if (this._socket == null || this._socket.InputStream == null)
                return null;
            ...
            {
                serverHash = new byte[this._serverMac.KeySize];
            ...
            if (this._serverMac != null)
            {
                ...
                //  Calculate packet hash
                IBuffer clientHashBuffer = CryptographicEngine.Sign(this._serverMac, CryptographicBuffer.CreateFromByteArray(clientHashData));
                byte[] clientHash;
                CryptographicBuffer.CopyToByteArray(clientHashBuffer, out clientHash);
        ...
        private void MessageListener()
        {
            try
            {
                while (this._socket != null && this._socket.InputStream != null)
                {
